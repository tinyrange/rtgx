...
func Marshal(program Program) ([]byte, bool) {
	...
	rootLen += nodeSize(importData)
	rootLen += nodeSize(symbolData)
	rootLen += nodeSize(declData)
	rootLen += nodeSize(declMetaData)
	rootLen += nodeSize(initOrderData)
	rootLen += nodeSize(constData)
	rootLen += nodeSize(funcData)
	rootLen += nodeSize(sigData)
	rootLen += nodeSize(stmtData)
	rootLen += nodeSize(typeData)
	rootLen += nodeSize(typeFieldData)
	rootLen += nodeSize(typeIfaceData)
	rootLen += nodeSize(typeFuncData)
	rootLen += nodeSize(methodData)
	rootLen += nodeSize(typeRefData)
	rootLen += nodeSize(localData)
	rootLen += nodeSize(indexData)
	rootLen += nodeSize(compData)
	rootLen += nodeSize(assignData)
	rootLen += nodeSize(returnData)
	rootLen += nodeSize(callData)
	rootLen += nodeSize(refData)
	...
	out = appendNode(out, TagImports, importData)
	out = appendNode(out, TagSymbols, symbolData)
	out = appendNode(out, TagDecls, declData)
	out = appendNode(out, TagDeclMeta, declMetaData)
	out = appendNode(out, TagInitOrder, initOrderData)
	out = appendNode(out, TagConsts, constData)
	out = appendNode(out, TagFuncs, funcData)
	out = appendNode(out, TagSigs, sigData)
	out = appendNode(out, TagStmts, stmtData)
	out = appendNode(out, TagTypes, typeData)
	out = appendNode(out, TagTypeFields, typeFieldData)
	out = appendNode(out, TagTypeIfaces, typeIfaceData)
	out = appendNode(out, TagTypeFuncs, typeFuncData)
	out = appendNode(out, TagMethods, methodData)
	out = appendNode(out, TagTypeRefs, typeRefData)
	out = appendNode(out, TagLocals, localData)
	out = appendNode(out, TagIndexes, indexData)
	out = appendNode(out, TagComps, compData)
	out = appendNode(out, TagAssigns, assignData)
	out = appendNode(out, TagReturns, returnData)
	out = appendNode(out, TagCalls, callData)
	out = appendNode(out, TagRefs, refData)
	out = appendNode(out, TagSels, selectorData)
	return out, true
}
