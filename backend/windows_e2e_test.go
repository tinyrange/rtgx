...
func TestWindowsTargetsEndToEnd(t *testing.T) {
	...
	testSources := []string{
		...
		"tests/windows_native_struct_layout.renvo",
	}
	amd64TestSources := []string{
		"tests/windows_amd64_linkstatic_even_stack_args.renvo",
	}
	...
	for _, target := range windowsE2ETargets {
		...
		t.Run(target.name, func(t *testing.T) {
			...
			t.Run("unit_frontend_smoke", func(t *testing.T) {
				unitDir := t.TempDir()
				unitPath := filepath.Join(unitDir, "smoke.unit")
				program, err := unit.ConvertFiles([]string{"tests/print_pass_smoke.go"})
				if err != nil {
...
func buildWindowsStage1(t *testing.T, stage0 string, target string, files []string, outDir string) string {
	t.Helper()
	stage1 := filepath.Join(outDir, "renvo-stage1.exe")
...
func runWindowsCommand(t *testing.T, dir string, path string, args ...string) (commandResult, error) {
	...
	if runtime.GOOS != "windows" || result.exitCode == 0 || os.Getenv("RENVO_WINDOWS_GDB") == "" {
		return result, err
	}
	gdbArgs := []string{
		"--batch",
