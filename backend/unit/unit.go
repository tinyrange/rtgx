...
func Marshal(program Program) ([]byte, error) {
	...
	root := Parent(TagUnit,
		...
		NewNode(TagImportPath, []byte(program.ImportPath)),
...
func Unmarshal(data []byte) (Program, error) {
	...
	for pos < rootEnd {
		...
		case TagImportPath:
			if seenImportPath {
				return program, fmt.Errorf("duplicate import path")
			}
			seenImportPath = true
		...
		case TagImports:
			if seenImports {
				return program, fmt.Errorf("duplicate import table")
			}
			seenImports = true
			importData = payload
		case TagSymbols:
...
func scanSource(src []byte) ([]sourceToken, int) {
	...
	for i < len(src) {
		...
		if c == '\'' {
			...
			toks = append(toks, sourceToken{kind: renvoTokChar, text: string(src[start:i]), line: line, start: start, end: i})
			continue
		}
		start := i
		i++
		...
		toks = append(toks, sourceToken{kind: renvoTokOp, text: string(src[start:i]), line: line, start: start, end: i})
	}
	toks = append(toks, sourceToken{kind: renvoTokEOF, line: line, start: len(src), end: len(src)})
	return toks, line
}
...
func parseTopLevel(toks []sourceToken) (string, []sourceDecl, []sourceFunc, error) {
	if len(toks) < 3 || toks[0].kind != renvoTokPackage || toks[1].kind != renvoTokIdent {
	...
	i := 0
	for i < len(toks) && toks[i].kind != renvoTokEOF {
		if toks[i].kind == renvoTokPackage {
			if i+1 >= len(toks) || toks[i+1].kind != renvoTokIdent {
				return "", nil, nil, fmt.Errorf("missing package name")
			}
			i += 2
			continue
		}
		if toks[i].kind == renvoTokConst || toks[i].kind == renvoTokVar || toks[i].kind == renvoTokType {
			start := i
			kind := toks[i].kind
			i++
			if tokCharIs(toks, i, '(') {
			...
			if i >= len(toks) || toks[i].kind != renvoTokIdent {
...
func parseFuncDecl(toks []sourceToken, start int) (sourceFunc, bool) {
	...
	for i < len(toks) && toks[i].kind != renvoTokEOF {
		...
		if tokCharIs(toks, i, '{') {
			if i > 0 && (toks[i-1].kind == renvoTokStruct || toks[i-1].kind == renvoTokIdent && toks[i-1].text == "interface") {
				next := skipBalanced(toks, i, '{', '}')
				if next <= i {
					return fn, false
				}
...
func decodeImports(data []byte) ([]Import, error) {
	...
	count, next, ok := readVarint(data, pos)
	if !ok {
		return nil, fmt.Errorf("invalid import count")
	}
	pos = next
	...
	for i := 0; i < count; i++ {
		...
		if !ok {
			return nil, fmt.Errorf("invalid import %d name", i)
		}
		pos = n
		importPath, n, ok := readString(data, pos)
		if !ok {
			return nil, fmt.Errorf("invalid import %d path", i)
		}
		pos = n
		pkg, n, ok := readNullable(data, pos)
		if !ok {
			return nil, fmt.Errorf("invalid import %d package", i)
		}
		pos = n
		nameTok, n, ok := readNullable(data, pos)
		if !ok {
			return nil, fmt.Errorf("invalid import %d name token", i)
		}
		pos = n
		pathTok, n, ok := readVarint(data, pos)
		if !ok {
			return nil, fmt.Errorf("invalid import %d path token", i)
		}
		pos = n
		flags, n, ok := readVarint(data, pos)
		if !ok || flags > 3 {
			return nil, fmt.Errorf("invalid import %d flags", i)
		}
		pos = n
		imp := Import{
			Name:       name,
		...
		if imp.Dot && imp.Blank {
			return nil, fmt.Errorf("invalid import %d dot blank flags", i)
		}
...
func readString(data []byte, pos int) (string, int, bool) {
	size, next, ok := readVarint(data, pos)
	if !ok {
		return "", pos, false
	}
	pos = next
