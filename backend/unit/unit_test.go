...
`))
	...
	if len(decoded.Decls) != 1 || decoded.Decls[0].Kind != renvoTokConst {
...
`)
	...
	if !bytes.Contains(program.Text, []byte("// renvo:linkstatic libc,puts")) {
...
`))
	...
	if len(decoded.Locals) != 1 || decoded.Locals[0].Kind != renvoTokVar || decoded.Locals[0].Values[0] != program.Locals[0].Values[0] {
