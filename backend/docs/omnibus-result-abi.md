...
target. The exported symbol is the deliberately short `renvores`; firmware must
place it in `.renvo.result` and retain it in the linked symbol table.
...
The host decoder finds `renvores` from the linked ELF rather than assigning it a
