# Renvo backend contract

RENVO uses a direct-emitter intermediate representation. The shared lowering
...
This contract is an internal RENVO ABI. It is not the platform C ABI, the Go ABI,
...
`renvoAsmCopyPrimaryToSecondary`, `renvoAsmLoadPrimaryStack`, and
`renvoAsmAddSecondaryTertiary`. Architecture implementation functions may use
...
`renvoBackendValueSlotSize` is eight bytes. It is the normalized compiler stack
and aggregate layout unit, not the target pointer width or target `int` width.
Machine properties come from `renvoTargetProfile`.
...
- Arrays are contiguous element storage. Their current call flattening uses
  ...
  contract and must not be inferred from `renvoBackendValueSlotSize`.
...
normalized to the declared method receiver before the ordinary RENVO call ABI is
...
aggregate before returning. Calls are internal RENVO calls unless a dedicated OS
