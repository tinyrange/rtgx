...
func renvoCompileOutputData(data []byte, target int) []byte {
	if !renvoCompilerEmitImage {
		return data
	}
	format := renvoLinkedImageFormat(data)
	...
	checkA, checkB := renvoLinkedImageChecksum(data)
...
func renvoLinkedImageFormat(data []byte) int {
	...
	if len(data) >= 4 && data[0] == 0 && data[1] == 'a' && data[2] == 's' && data[3] == 'm' {
		return 4
	}
	return 0
}

func renvoLinkedImageChecksum(data []byte) (int, int) {
	a := 1
	b := 0
...
func renvoLinearInitGlobal(g *renvoLinearGen, index int) bool {
	...
	skipInitializer := -1
	if renvoFixedTarget == 0 {
		skipInitializer = renvoLinearReplInitializerSkip(g, index)
	}
...
func renvoLinearReplInitializerSkip(g *renvoLinearGen, index int) int {
	renvoNonNil(g)
...
func renvoLinearPrepareReplGlobals(g *renvoLinearGen) {
	...
	if renvoFixedTarget != 0 || !renvoCompilerEmitImage ||
		renvoTargetArch == renvoArchWasm32 {
		return
	}
	for i := 0; i < len(g.meta.globals); i++ {
		s := &g.meta.globals[i]
...
func renvoEmitPointerAssignment(g *renvoLinearGen, left *renvoExprParse, pointerIndex int, right *renvoExprParse, rightIndex int, targetType int, assignTok int) bool {
	...
	if !renvoEmitIntExpr(g, left, pointerIndex) {
		return false
	}
	renvoEmitRuntimeNonNilPrimary(g)
	...
	renvoAsmPushPrimary(a)
	renvoAsmCopyPrimaryToSecondary(a)
	renvoAsmLoadPrimaryMemSecondaryDispSize(a, 0, renvoScalarKindSize(kind))
	renvoAsmPushPrimary(a)
	if !renvoEmitScalarExprForKind(g, right, rightIndex, kind) {
		return false
	}
	renvoAsmPopTertiary(a)
	if !renvoEmitTypedPrimaryTertiaryOp(g, assignTok, kind == renvoTypeFloat64) {
		return false
	}
	renvoAsmNormalizePrimaryForKind(a, kind)
	renvoAsmPopSecondary(a)
	renvoAsmStorePrimaryMemSecondaryDispSize(a, 0, renvoScalarKindSize(kind))
	return true
}
...
func renvoEmitLinearAssign(g *renvoLinearGen, stmt *renvoStmt) bool {
	...
	if compoundAssign {
		...
		renvoAsmPopTertiary(a)
		if !renvoEmitTypedPrimaryTertiaryOp(g, assignTok, targetResolved.kind == renvoTypeFloat64) {
			return false
		}
...
func renvoEmitUserCall(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	...
	if fn.linkStatic != 0 {
		...
		} else if renvoTargetOS == renvoOSWindows {
			if renvo_runtime_UnsafeByteAt(g.prog.src, fn.linkDLLStart) != '/' {
				return renvoEmitLinkStaticCall(g, fn, wordCount)
			}
		}
...
func renvoEmitAppendExpansionToLocation(g *renvoLinearGen, ep *renvoExprParse, locEp *renvoExprParse, loc *renvoSliceLocation, elemType int, valueIndex int) bool {
	...
	if elemSize == 1 && renvoTargetArch == renvoArchAmd64 {
...
func renvoZeroLocalAtOffset(g *renvoLinearGen, offset int) {
	...
	} else if renvoNativeIntSize == 8 && size >= 24 {
		renvoAsmAddressPrimaryStack(a, offset)
...
func renvoLinearPersistentCapacity(g *renvoLinearGen) int {
	...
	return cap(a.code) + cap(a.labelPos) + cap(a.relocs) + cap(a.absRelocs) + cap(a.symbols) + cap(a.symbolName) + cap(a.winImports) + cap(a.darwinImports) + cap(a.darwinImportLabels) + cap(a.darwinImportUsed) + cap(a.data) + objectStringCapacity + cap(g.breakLabels) + cap(g.continueLabels) + cap(m.types) + cap(m.fields) + cap(m.captures)
}
...
func renvoEmitTypedPrimaryTertiaryOp(g *renvoLinearGen, tok int, float bool) bool {
	renvoNonNil(g)
	a := &g.asm
	if float && renvoTok2Is(g.prog, tok, '*', '=') {
...
func renvoEmitIntExpr(g *renvoLinearGen, ep *renvoExprParse, idx int) bool {
	...
	if idx >= 0 && idx < len(ep.exprs) && ep.exprs[idx].kind == renvoExprCall {
		if renvoFixedTarget == 0 {
			if renvoExprIsIdentText(g.prog, ep, ep.exprs[idx].left, "renvo_runtime_CallJIT") {
