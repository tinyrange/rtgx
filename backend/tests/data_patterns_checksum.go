...
func appMain(args []string) int {
	...
	records = append(records, symbolRecord{value: 2, tag: 'a'})
	records = append(records, symbolRecord{value: 3, tag: 'b'})
	checksum := 0
	i := 0
	for i < len(records) {
		checksum += records[i].value + int(records[i].tag)
		i += 1
	}
	if checksum != 200 {
		print("RENVO-0893 checksum failed\n")
		return 1
	}
	print("PASS\n")
	return 0
