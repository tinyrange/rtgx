...
func appMain(args []string) int {
	globalRecords = append(globalRecords, symbolRecord{value: 1, next: 1})
	globalRecords = append(globalRecords, symbolRecord{value: 2, next: 2})
	globalRecords = append(globalRecords, symbolRecord{value: 4, next: -1})
	if walkNext(globalRecords, 0) != 7 {
		print("RENVO-0897 recursive walk failed\n")
		return 1
	}
	print("PASS\n")
	return 0
