...
func appMain(args []string) int {
	r := mixRecord{text: "mix", b: 'A', ok: true}
	r.data = append(r.data, byte(2))
	score := r.n + int(r.b) + len(r.text) + len(r.data) + int(r.data[0])
	if r.ok {
		score += 5
	}
	if score != 76 {
		print("RENVO-0975 final mixed checksum failed\n")
		return 1
	}
	print("PASS\n")
	return 0
