...
func appMain(args []string) int {
	...
	nodes = append(nodes, symbolRecord{value: 1, parent: -1})
	nodes = append(nodes, symbolRecord{value: 2, parent: 0})
	nodes = append(nodes, symbolRecord{value: 3, parent: 1})
	i := 2
	sum := 0
	for i >= 0 {
		sum += nodes[i].value
	...
	if sum != 6 {
		print("RENVO-0884 parent index failed\n")
		return 1
	}
	print("PASS\n")
	return 0
