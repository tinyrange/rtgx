...
func TestLayoutIsSmallAndLittleEndian(t *testing.T) {
	...
	if string(block[OffsetMagic:OffsetMagic+4]) != "RNVR" {
...
func TestGeneratedDefinitionsAreCurrent(t *testing.T) {
	command := exec.Command("go", "run", "../../cmd/renvoresultgen", "-spec", "protocol.json", "-go", "layout_generated.go", "-c", "result_abi.h", "-check")
...
func TestC89HeaderAndELFSymbol(t *testing.T) {
	...
	program := "#include \"result_abi.h\"\nunsigned char renvores[RENVO_RESULT_SIZE];\nint renvo_result_fixture(void) { return (int)RENVO_RESULT_OFFSET_SIGNATURE; }\n"
