...
func run(args []string, stdout io.Writer, stderr io.Writer) int {
	flags := flag.NewFlagSet("rtgc89", flag.ContinueOnError)
	...
	intBits := flags.Int("int", 0, "RTG language int width for explicit mode: 16, 32, or 64")
	...
	flags.Usage = func() {
		fmt.Fprintln(flags.Output(), "usage: rtgc89 [options]")
	...
	if flags.NArg() != 0 {
		fmt.Fprintf(stderr, "rtgc89: unexpected argument %q\n", flags.Arg(0))
		return 2
	}
	endian, ok := parseEndian(*endianName)
	if !ok {
		fmt.Fprintf(stderr, "rtgc89: invalid endianness %q; want little or big\n", *endianName)
		return 2
	}
	...
	case "automatic":
		if *intBits != 0 || *pointerBits != 0 {
			fmt.Fprintln(stderr, "rtgc89: -int and -pointer are only valid in explicit mode")
			return 2
		}
		profile = target.C89AutomaticProfile(*name, *hosted, endian, *abi, runtimeOps...)
	case "explicit":
		if *intBits == 0 || *pointerBits == 0 {
			fmt.Fprintln(stderr, "rtgc89: explicit mode requires -int and -pointer")
			return 2
		}
		profile = target.C89ExplicitProfile(*name, *hosted, *intBits, *pointerBits, endian, *abi, runtimeOps...)
	default:
		fmt.Fprintf(stderr, "rtgc89: invalid mode %q; want automatic or explicit\n", *mode)
		return 2
	}
	var source []byte
	var err error
	...
	if err != nil {
		fmt.Fprintf(stderr, "rtgc89: %v\n", err)
		return 2
	}
	if *output == "-" {
		if _, err := stdout.Write(source); err != nil {
			fmt.Fprintf(stderr, "rtgc89: write stdout: %v\n", err)
			return 1
		}
		return 0
	}
	if err := os.WriteFile(*output, source, 0o644); err != nil {
		fmt.Fprintf(stderr, "rtgc89: write %s: %v\n", *output, err)
		return 1
	}
	return 0
}
