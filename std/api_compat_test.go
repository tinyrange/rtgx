...
func TestHostAndRenvoCommonAPIsMatch(t *testing.T) {
	root := stdRoot(t)
	...
	for _, target := range targets {
		...
		t.Run(target.goos+"_"+target.goarch, func(t *testing.T) {
			...
			renvoContext := hostContext
			renvoContext.BuildTags = append([]string{}, hostContext.BuildTags...)
			renvoContext.BuildTags = append(renvoContext.BuildTags, "renvo")
			for _, dir := range packages {
				...
				renvo := loadPublicAPI(t, dir, &renvoContext)
				for name, hostDeclaration := range host {
					renvoDeclaration, ok := renvo[name]
					if !ok {
						continue
					}
					compared++
					...
					if hostDeclaration.signature != renvoDeclaration.signature {
						t.Errorf("%s: host %s (%s), renvo %s (%s)", key, hostDeclaration.signature, hostDeclaration.file, renvoDeclaration.signature, renvoDeclaration.file)
					}
				}
			}
		})
	...
	if compared == 0 {
		t.Fatal("no common host/RENVO declarations were compared")
	}
}

func publicAPIException(key string) bool {
	// RENVO's unsafe.Pointer is a compiler-recognized data pointer. The host shim
