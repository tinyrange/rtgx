...
func TestBackendRTGUGoldenVectors(t *testing.T) {
	for _, name := range []string{"v1-core.hex", "v1-full.hex"} {
		data := readBackendGolden(t, "rtgunit/testdata/"+name)
		program, isUnit, ok := rtgDecodeUnitProgram(data)
...
func TestBackendRTGUVersionOneCompatibility(t *testing.T) {
	core := readBackendGolden(t, "rtgunit/testdata/v1-core.hex")
	unknown := appendBackendUnitChild(core, 60000, []byte{1, 2, 3})
	if _, isUnit, ok := rtgDecodeUnitProgram(unknown); !isUnit || !ok {
		t.Fatal("backend rejected unknown optional tag")
	}

	version := append([]byte(nil), core...)
	binary.LittleEndian.PutUint16(version[4:6], rtgunit.Version+1)
	if _, isUnit, ok := rtgDecodeUnitProgram(version); !isUnit || ok {
		t.Fatal("backend accepted unsupported version")
	}

	flags := append([]byte(nil), core...)
	binary.LittleEndian.PutUint16(flags[6:8], 1)
	if _, isUnit, ok := rtgDecodeUnitProgram(flags); !isUnit || ok {
		t.Fatal("backend accepted nonzero flags")
	}
}

func TestBackendRTGURejectsMissingDuplicateAndMalformedData(t *testing.T) {
	core := readBackendGolden(t, "rtgunit/testdata/v1-core.hex")
	for _, item := range rtgunit.WireSchemaTags {
		if item.Role != "child" || !item.Required {
			continue
		}
		without := removeBackendUnitChild(t, core, item.Number)
		if _, isUnit, ok := rtgDecodeUnitProgram(without); !isUnit || ok {
			t.Errorf("backend accepted missing required tag %s", item.Name)
		}
	}

	full := readBackendGolden(t, "rtgunit/testdata/v1-full.hex")
	for _, item := range rtgunit.WireSchemaTags {
		if item.Role != "child" {
			continue
		}
		...
		if _, isUnit, ok := rtgDecodeUnitProgram(duplicate); !isUnit || ok {
			t.Errorf("backend accepted duplicate tag %s", item.Name)
		}
	}
	for _, payload := range [][]byte{{0x80, 0x00}, {0xff, 0xff, 0xff, 0xff, 0x10}} {
		badTokens := replaceBackendUnitChild(t, core, rtgunit.TagTokens, payload)
		if _, isUnit, ok := rtgDecodeUnitProgram(badTokens); !isUnit || ok {
			t.Errorf("backend accepted invalid token count varint %x", payload)
		}
	}

	badDecl := replaceBackendUnitChild(t, core, rtgunit.TagDecls, []byte{1, rtgTokVar, 99, 1, 0, 1})
	if _, isUnit, ok := rtgDecodeUnitProgram(badDecl); !isUnit || ok {
		t.Fatal("backend accepted declaration outside text/token ranges")
	}

	for end := 4; end < len(core); end++ {
		if _, isUnit, ok := rtgDecodeUnitProgram(core[:end]); !isUnit || ok {
	...
	if _, isUnit, ok := rtgDecodeUnitProgram(overflow); !isUnit || ok {
