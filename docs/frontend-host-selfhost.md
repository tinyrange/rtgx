...
building, lowering, linking, and RTGU encoding live in untagged files under:
...
- `rtg/internal/driver` uses Go host APIs in stage0 and target runtime calls in
  a self-hosted executable for file discovery, process startup, and backend
  invocation.
- `rtg/internal/load` has a target-specific absolute-path adapter where the
  host standard library and the RTG runtime expose different primitives.
- `rtg/internal/backendbridge` selects the native in-process backend bridge by
  operating system and architecture.
- `rtg/internal/arena` is harmless on the Go host. The RTG runtime recognizes
  its intrinsic calls and uses them to reclaim transient compiler storage.
- Packages under `rtg/std` may use host-library shims or target runtime calls.
  Their common public API is checked by `rtg/std/api_compat_test.go`; graphics,
  filesystem, process, and unsafe implementations remain platform boundaries.
- `rtg_bundle` controls whether standard-library source is embedded in a host
