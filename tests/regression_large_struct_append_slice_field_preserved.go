...
func appMain() int {
	...
	if len(packages) != 2 {
		print("RTG-1141 package append length failed\n")
		return 1
	}
	copied := copyFiles(packages[1].files)
	sortFilesByPath(copied)
	if len(copied) != 2 {
		print("RTG-1141 file copy length failed\n")
		return 1
	}
	if copied[0].path != "a.go" || copied[1].path != "z.go" {
		print("RTG-1141 file sort path corrupted\n")
		return 1
	}
	if len(copied[0].parsed.tokens) != 2 || copied[0].parsed.tokens[1].text != "main" {
		print("RTG-1141 nested token slice corrupted\n")
		return 1
	}
	print("PASS\n")
	return 0
