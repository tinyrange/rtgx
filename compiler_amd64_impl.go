...
func rtgAmd64EmitCompareJump(g *rtgLinearGen, ep *rtgExprParse, e *rtgExpr, label int, jumpIfTrue bool) bool {
	...
	if rightConst.ok && rtgAsmImmFits8Signed(rightConst.value) {
	...
	if rtgTargetArch == rtgArchAmd64 {
		...
		if left.kind == rtgExprIdent && right.kind == rtgExprIdent {
			leftLocal := rtgFindLocalIndex(g, left.nameStart, left.nameEnd)
			rightLocal := rtgFindLocalIndex(g, right.nameStart, right.nameEnd)
			if leftLocal >= 0 && rightLocal >= 0 && rtgTypeIsInt(g.meta, g.locals[leftLocal].typ) && rtgTypeIsInt(g.meta, g.locals[rightLocal].typ) {
				rtgAsmLoadPrimaryStack(&g.asm, g.locals[rightLocal].offset)
				rtgAsmStackMem(&g.asm, g.locals[leftLocal].offset, 0x3948, 0x45, 0x85)
				rtgEmitCompareJumpOp(&g.asm, c0, c1, label, jumpIfTrue)
				return true
			}
		}
...
func rtgAmd64EmitIntExpr(g *rtgLinearGen, ep *rtgExprParse, idx int) bool {
	...
	if e.kind == rtgExprBinary {
		...
		if rightKind == rtgExprInt {
			...
			immediate = true
		} else if rightKind == rtgExprIdent {
			value = rtgFindSmallConstByName(g, rightExpr.nameStart, rightExpr.nameEnd)
			immediate = value >= -128
		}
		if rtgTargetArch == rtgArchAmd64 && immediate && (immOpcode != 0 || immMultiply || immShift != 0) {
			if value >= -2147483647 && value <= 2147483647 && (immShift == 0 || value >= 0 && value < 64) {
				if !rtgEmitIntExpr(g, ep, e.left) {
					return false
				}
				if immShift != 0 {
					rtgAsmEmit4(a, 0x48, 0xc1, immShift, value)
				} else if immMultiply && rtgAsmImmFits8Signed(value) {
					rtgAsmEmit4(a, 0x48, 0x6b, 0xc0, value)
				} else if immMultiply {
					rtgAsmEmit3(a, 0x48, 0x69, 0xc0)
					rtgAsmEmit32(a, value)
				} else if rtgAsmImmFits8Signed(value) {
					rtgAsmEmit4(a, 0x48, 0x83, immGroup, value)
				} else {
					rtgAsmEmit2(a, 0x48, immOpcode)
					rtgAsmEmit32(a, value)
				}
				rtgAmd64NormalizeExprPrimary(g, ep, idx)
				return true
...
func rtgAmd64EmitFloatBinaryExpr(g *rtgLinearGen, ep *rtgExprParse, idx int) bool {
	...
	a := &g.asm
	e := &ep.exprs[idx]
	multiply := rtgTokCharIs(p, e.tok, '*')
	divide := rtgTokCharIs(p, e.tok, '/')
...
func rtgAmd64EnsureAppend8Helper(g *rtgLinearGen) int {
	...
	rtgAsmEmitText(a, "\x48\x8b\x0e\x4c\x8b\x07\x41\x88\x14\x08\x48\xff\xc1\x48\x89\x0e\xc3")
	rtgAsmMarkLabel(a, afterLabel)
	return g.append8Label
}
func rtgAmd64EnsureAppend64Helper(g *rtgLinearGen) int {
	...
	rtgAsmEmitText(a, "\x48\x8b\x0e\x4c\x8b\x07\x49\x89\x14\xc8\x48\xff\xc1\x48\x89\x0e\xc3")
	rtgAsmMarkLabel(a, afterLabel)
	return g.append64Label
}
...
func rtgAmd64EnsureStringEqualHelper(g *rtgLinearGen) int {
	...
	rtgAsmEmitText(a, "\x31\xc0\x48\x39\xce")
	rtgAsmJnzLabel(a, notEqualLabel)
	rtgAsmEmitText(a, "\x48\x85\xf6")
	rtgAsmJzLabel(a, equalLabel)
	rtgAsmMarkLabel(a, loopLabel)
	rtgAsmEmitText(a, "\x44\x8a\x07\x44\x38\x02")
	rtgAsmJnzLabel(a, notEqualLabel)
	rtgAsmEmitText(a, "\x48\xff\xc7\x48\xff\xc2\x48\xff\xce")
	rtgAsmJnzLabel(a, loopLabel)
	rtgAsmMarkLabel(a, equalLabel)
	rtgAsmEmitText(a, "\x6a\x01\x58")
	rtgAsmMarkLabel(a, notEqualLabel)
	rtgAsmEmitText(a, "\xc3")
	rtgAsmMarkLabel(a, afterLabel)
	return g.streqLabel
}
