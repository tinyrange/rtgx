...
`)},
	...
	if _, err := wireunit.Unmarshal(linked.Data); err != nil {
		t.Fatalf("linked unit did not decode: %v", err)
	}
}
...
`)},
	...
	if !bytes.Contains(linked.Program.Text, []byte("func appMain(args []string, env []string) int")) ||
		!bytes.Contains(linked.Program.Text, []byte("renvo_runtime_SetProcess(args, env)")) ||
		!bytes.Contains(linked.Program.Text, []byte("main()")) {
...
func functionName(program wireunit.Program, fn wireunit.Func) string {
