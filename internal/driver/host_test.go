...
func TestCompileAndWriteEmitsUnitWithoutBackend(t *testing.T) {
	...
	result := CompileAndWriteWithEnv([]string{"-emit-unit", "-o", "program.unit", "./cmd/app"}, nil, nil)
	if !result.Ok {
		t.Fatalf("unit emission failed: %#v", result)
	}
	data, err := os.ReadFile("program.unit")
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(data, result.Compile.Build.Unit) || !bytes.HasPrefix(data, []byte("RNVO")) {
		t.Fatalf("unit output does not match the linked unit: %x", data)
	}
	info, err := os.Stat("program.unit")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunCommandDropsProgramName(t *testing.T) {
	...
	backend := &recordingBackend{binary: []byte("binary")}
	result := RunCommand([]string{"renvo", "-o", "app", "./cmd/app"}, nil, backend)
	if !result.Ok {
...
func TestRunCommandWithRealBackendCompilesArrayLiterals(t *testing.T) {
	runHostMainCase(t, []byte(`package main
...
func runHostMainCase(t *testing.T, src []byte) {
	...
	cmd := exec.Command("go", "build", "-o", backend, "./backend")
	cmd.Dir = repoRoot
