...
func TestCommandBackendCompileUnit(t *testing.T) {
	...
	backend := CommandBackend{
		...
		Env:  []string{"RENVO_DRIVER_COMMAND_BACKEND_HELPER=1"},
	}
...
func TestCommandBackendFailure(t *testing.T) {
	...
	backend := CommandBackend{
		Path: exe,
		Args: []string{"-test.run=TestCommandBackendHelper", "--"},
		Env:  []string{"RENVO_DRIVER_COMMAND_BACKEND_HELPER=1", "RENVO_DRIVER_COMMAND_BACKEND_FAIL=1"},
	}
	result := backend.CompileUnit(built.Unit, "linux/amd64", false, false)
	if result.Ok {
		t.Fatal("failing backend was accepted")
	}
	if result.Diagnostic.Code != "RENVO-BACKEND-003" {
...
func TestCommandBackendHelper(t *testing.T) {
	if os.Getenv("RENVO_DRIVER_COMMAND_BACKEND_HELPER") != "1" {
		return
	}
	if os.Getenv("RENVO_DRIVER_COMMAND_BACKEND_FAIL") == "1" {
