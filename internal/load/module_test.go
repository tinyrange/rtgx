...
module example.com/renvotests/quick/packages/case000
...
`))
	...
	if mod.Path != "example.com/renvotests/quick/packages/case000" {
...
module "example.com/renvotests/quoted"
`))
	if !mod.Ok {
		t.Fatalf("ParseModule failed: err=%d offset=%d", mod.Error, mod.ErrorOffset)
	}
	...
	if mod.Path != "example.com/renvotests/quoted" {
		t.Fatalf("module path = %q", mod.Path)
	}
}
...
func TestResolveImport(t *testing.T) {
	mod := Module{Root: "/repo/case", Path: "example.com/case", Ok: true}
	local := ResolveImport(mod, "/std", "example.com/case/pkg/lib")
	...
	std := ResolveImport(mod, "/std", "runtime")
	if !std.Ok || std.Kind != PackageStandard || std.Dir != "/std/runtime" {
		t.Fatalf("std import = %#v", std)
	}
	external := ResolveImport(mod, "/std", "other.example/pkg")
	if external.Ok || external.Error != ResolveErrUnsupported {
		t.Fatalf("external import = %#v", external)
	}
	relative := ResolveImport(mod, "/std", "./pkg")
...
func TestFileImports(t *testing.T) {
	mod := Module{
		Root: "/repo/case",
		Path: "example.com/renvotests/quick/packages/case000",
		Ok:   true,
	}
...
`)
	...
	imports := FileImports(mod, "/std", file)
	if len(imports) != 2 {
		t.Fatalf("import count = %d, want 2", len(imports))
	}
	if !imports[0].Ok || imports[0].Kind != PackageStandard || imports[0].Dir != "/std/runtime" {
